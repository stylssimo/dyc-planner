...
import { Search, Users, Filter, MoreHorizontal, X, Check, Loader2 } from 'lucide-react';
...
const AdminCons = () => {
    ...
    const setButtonLoading = (id: string, action: 'consult' | 'success' | 'reject', isLoading: boolean) => {
    ...
    const addConsultationToCalendar = async (consultation: TableRow) => {
        try {
            if (!user?.email) {
            ...
            // Get consultant's calendar document
            ...
            const newCalendarEvent = {
                ...
                date: consultation.consultationDate, // e.g., "2025-03-21"
                notes: `Interview with ${consultation.userEmail}`, // or however you want to format it
                time: consultation.consultationTime, // e.g., "10:00"
                title: `Interview with ${consultation.userEmail}`, // or use consultation.tripName
                type: "consultation",
            ...
            const updatedEvents = [...existingEvents, newCalendarEvent];
    
            // Update the calendar document (or create it if it doesn't exist)
            await setDoc(userCalendarRef, {
                userEmail: user.email,
                events: updatedEvents,
                createdAt: userCalendarSnap.exists() ? userCalendarSnap.data().createdAt : new Date().toISOString(),
                updatedAt: new Date().toISOString()
            });
    
            console.log('Calendar event added successfully:', newCalendarEvent);
        } catch (error) {
            console.error('Error adding calendar event:', error);
            // Don't throw error to prevent consultation update from failing
        }
    };
    ...
    const handleConsultConsultation = async (id: string) => {
        setButtonLoading(id, 'consult', true);
        try {
            ...
            // Add to calendar (simple add, no matching needed)
            await addConsultationToCalendar(consultation);
            
            // Update local state to reflect the change
            setConsultationsData(prev => 
                prev.map(item => 
                    item.id === id ? { ...item, status: 'Active' as TableRow['status'] } : item
                )
            );
            ...
            setButtonLoading(id, 'consult', false);
        }
    };
    ...
    const handleSuccessConsultation = async (id: string) => {
        ...
        //     await updateEvent(consultation, 'Consulted');
            ...
            alert('Failed to mark consultation as completed. Please try again.');
        } finally {
    ...
    const handleRejectConsultation = async (id: string) => {
        ...
        try {
            ...
            // Update calendar event
        //     await updateEvent(consultation, 'Cancelled');
                                ...
                                <table className="w-full">
									...
									<div className="ml-3">
										<div className="text-sm font-medium text-gray-900">{row.userEmail}</div>
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{row.userPhoneNumber || 'N/A'}</td>
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                                <div className="flex items-center space-x-2">
                                                                                                                                <button
                                                                onClick={() => handleConsultConsultation(row.id)}
                                                                disabled={loadingStates[row.id]?.consult || row.status === 'Consulted' || row.status === 'Cancelled'}
                                                                ...
                                                                title={row.status === 'Consulted' ? 'Already consulted' : row.status === 'Cancelled' ? 'Consultation cancelled' : 'Start consultation'}
                                                                >
                                                                {loadingStates[row.id]?.consult ? (
